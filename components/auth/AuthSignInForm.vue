<template>
  <div class="auth-sign-in-form">
    <form class="auth-sign-in-form__form">
      <div class="auth-sign-in-form__group">
        <ui-input
          ...
          border-radius="big"
          placeholder="Email"
          class="auth-sign-in-form__input auth-sign-in-form__input--email"
          @input="v$.email.$reset"
					:aria-label="$t('Email')"
        />
        <form-input-password
    ...
    <div class="auth-sign-in-form__caption text-regular-19-19">
      {{ $t("auth.noAccount") }}
      <button
        class="auth-sign-in-form__caption-btn"
        @click="emit('change-auth-state', AuthState.SIGN_UP)"
      >
        {{ $t("Join") }}
      </button>
    </div>
...
async function signIn() {
  ...
  try {
    await auth.loginWith("cookie", { body: user });
    emit("on-auth");
    emit("close-modal");
  } catch (error) {
