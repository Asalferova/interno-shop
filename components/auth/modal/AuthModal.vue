<template>
	<centered-modal class="auth-modal" @update:model-value="(val) => emit('update:modelValue', val)">
		<template #title>
		...
		<transition name="fade" mode="out-in">
			<component :is="currentAuthForm" @change-auth-state="changeAuthForm" @error-message="setErrorMessage"
				@close-modal="emit('update:modelValue', false)" @on-auth="doAfterAuth" />
		</transition>
	</centered-modal>
</template>
