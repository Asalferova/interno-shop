<template>
	<div ref="filter" class="filters-price-range" :class="`filters-price-range--${isOpen ? 'opened' : 'closed'}`"
		...
		<div class="filters-price-range__selected" @click="toggleFilter">
			<div class="filters-price-range__label">
				<span class="filters-price-range__label-default">{{ $t(props.defaultLabel) }}{{ priceText }}</span>
			</div>
			<ui-svg v-if="!props.fromValue && !props.toValue" name="arrow" class="filters-price-range__icon" />
			<ui-svg v-if="props.fromValue || props.toValue" name="cross" class="filters-price-range__icon--close"
				...
				<div class="filters-price-range__range-wrapper">
					<div>
						...
						<div class="filters-price-range__range-container">
							<ui-input v-model="fromModel" :size="'small'" :only-number="true" placeholder="0" min="0"
					...
					<div>
						...
						<div class="filters-price-range__range-container">
							<ui-input v-model="toModel" size="small" :only-number="true" min="0" placeholder="0"
								class="filters-price-range__range-input" />
						</div>
					</div>
				</div>
		...
		<div v-if="!props.hasInteractiveLabel && (props.fromValue || props.toValue)" class="filters-price-range__marker" />
	</div>
</template>
...
const priceText = computed(() => {
	...
	if (props.fromValue && props.toValue) {
		...
		)} — ${getPriceFormat(+props.toValue, currentCurrency.value)}`;
		...
		result = `: ${t("to")} ${getPriceFormat(+props.toValue, currentCurrency.value)}`;
	}
	return result;
});
