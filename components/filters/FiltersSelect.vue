<template>
	<div ref="filter" class="filters-select" :class="`filters-select--${isOpen ? 'opened' : 'closed'}`"
		:tabindex="tabindex" itemprop="additionalType">
		<div class="filters-select__selected" @click="toggleFilter">
			{{ categoryText }}

			<ui-svg v-if="!props.selectedOption" name="arrow" class="filters-select__icon" />
			<ui-svg v-if="props.selectedOption" name="cross" class="filters-select__icon--close"
				@click.stop="emit('close')" />
		</div>
		<transition name="dropdown">
			<nav v-if="isOpen" class="filters-select__options scrollable-component">
				<li v-for="(option, i) of options" :key="`filters-select-option-${i}`" class="filters-select__option"
					@click="onOptionSelect(option.label)">
					{{ $t(`subCategories.${option.label}`) }}
				</li>
			</nav>
		</transition>
		<div v-if="!props.hasInteractiveLabel && props.selectedOption" class="filters-select__marker" />
	</div>
</template>
...
import type { FiltersSubCategoryOption } from "~/helpers/filterOptions";
