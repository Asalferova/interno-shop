<template>
	<component :is="props.href ? 'a' : props.to ? NuxtLink : 'button'" class="ui-button" :class="[
		`ui-button--${props.color}`,
		`ui-button--${props.size}`,
		{ 'ui-button--icon': !!props.prependIcon || !!props.appendIcon },
	]" :disabled="disabled" v-bind="bindAttributes()" @click="emit('click', $event)">
		<ui-svg v-if="prependIcon" :name="prependIcon" class="ui-button__icon ui-button__icon--left" />
		<slot />
		<ui-svg v-if="appendIcon" :name="appendIcon" class="ui-button__icon ui-button__icon--right" />
	</component>
</template>
